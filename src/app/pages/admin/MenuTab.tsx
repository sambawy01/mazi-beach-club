...
export function MenuTab({ l }: { l: AdminLang }) {
  ...
  async function handleSave(data: Record<string, string>) {
    ...
    if (!pw) return;
    if (editItem) {
      await editMenuItem(pw, editItem.id, data);
  ...
  async function handleDelete(item: AdminItem) {
    ...
    if (!pw) return;
    try {
      await deleteMenuItem(pw, item.id);
  ...
  async function handleToggleVisibility(item: AdminItem) {
    ...
    try {
      await toggleItemVisibility(pw, item.id, newStatus);
      await fetchItems();
  ...
  return (
    <div>
      ...
      <ItemFormDialog key={editItem?.id ?? 'new'} open={dialogOpen} onOpenChange={setDialogOpen} item={editItem} sheetType="Menu" onSave={handleSave} l={l} />
    </div>
  );
}
